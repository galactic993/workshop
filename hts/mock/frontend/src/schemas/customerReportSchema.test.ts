...
import { customerReportSchema } from './customerReportSchema';

describe('customerReportSchema', () => {
  // 有効なフォームデータ
  const validFormData = {
    center_id: '1',
  ...
  describe('正常系', () => {
    it('有効な値でバリデーション通過', () => {
      const result = customerReportSchema.safeParse(validFormData);
      expect(result.success).toBe(true);
    });

    it('include_aggregatedがtrueでも通過', () => {
      const result = customerReportSchema.safeParse({
        ...validFormData,
